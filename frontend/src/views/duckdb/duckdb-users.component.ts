/**
 * DuckDB Users Component
 * Uses standardized serialization for backend communication
 */
...
import { RequestBuilder, isUser, isValidEmail, isValidString, isValidNumberRange } from '../../app/shared/serialization';
  ...
  async saveUser(): Promise<void> {
    ...
    try {
      ...
      if (!isValidEmail(data.email || '')) {
        this.logger.error('Validation failed: Invalid email');
        this.isLoading.set(false);
        return;
      }
      if (!isValidNumberRange(data.age || 0, 1, 150)) {
        this.logger.error('Validation failed: Age must be between 1 and 150');
        this.isLoading.set(false);
        return;
      }
      ...
      if (this.editingUser()) {
        ...
        const request = RequestBuilder.updateUser(
          this.editingUser()!.id,
          data.name || '',
          data.email || '',
          data.age || 25
        );
        await this.api.callOrThrow('updateUser', [request]);
